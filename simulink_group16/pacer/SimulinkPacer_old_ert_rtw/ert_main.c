...
#include "SimulinkPacer_old_private.h"
#include "rtwtypes.h"
#include "limits.h"
#include "board.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(SimulinkPacer_old_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    SimulinkPacer_old_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(SimulinkPacer_old_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(SimulinkPacer_old_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(SimulinkPacer_old_M, 0);

  /* Initialize model */
  SimulinkPacer_old_initialize();
