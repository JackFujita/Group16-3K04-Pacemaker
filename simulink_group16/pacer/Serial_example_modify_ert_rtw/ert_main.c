...
#include "Serial_example_modify_private.h"
#include "rtwtypes.h"
#include "limits.h"
#include "board.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Serial_example_modify_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Serial_example_modify_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Serial_example_modify_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Serial_example_modify_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Serial_example_modify_terminate();
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Serial_example_modify_M, 0);

  /* Initialize model */
  Serial_example_modify_initialize();
