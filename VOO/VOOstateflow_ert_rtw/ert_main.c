...
#include "VOOstateflow_private.h"
#include "rtwtypes.h"
#include "limits.h"
#include "board.h"
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(VOOstateflow_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    VOOstateflow_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(VOOstateflow_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(VOOstateflow_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(VOOstateflow_M, 0);

  /* Initialize model */
  VOOstateflow_initialize();
